...
def get_image_extractor(arch='', pretrained=True, feature_dim=None, checkpoint=''):
    '''
    Inputs
    ...
    if arch == 'ViT':
        model = vit_base()
        sta_dict = torch.load('utils/dino_vitbase16_pretrain.pth')
        model.load_state_dict(sta_dict, strict=True)
